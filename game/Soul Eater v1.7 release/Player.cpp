...
void Player::loadFX(int charactor)
{
	if (charactor == 0)
	{
		attackSoundFX.loadFromFile("Res/sound/blueAtk.ogg");
		damage = 10; defense = 10;
	}
	else if (charactor == 1)
	{
		attackSoundFX.loadFromFile("Res/sound/pinkAtk.ogg");
		damage = 15; defense = 5;
	}
	else if (charactor == 2)
	{
		attackSoundFX.loadFromFile("Res/sound/yellowAtk.ogg");
		damage = 20; defense = 0;
	}
	else if (charactor == 3)
	{
...
void Player::Update(int stage, float time, int charactor)
{
	// ALL Movement -----------
	if (!cutScene)
	{
		...
		if (Keyboard::isKeyPressed(Keyboard::Left) and !getHit and !Dead)
		{
			...
			else if (charactor == 1 or charactor == 2)
			{
				if ((isAttack or Dash) and (!jumping and !falling))
					velocity.x = 0.f;
				else if (isAttack)
		...
		if (Keyboard::isKeyPressed(Keyboard::Z) and !isAttack and !getHit and !Dead and !isPressed)
		{
			isPressed = true;
			attackFX.play();
			isAttack = true;
		...
		if (charactor == 0 or charactor == 1 or charactor == 2)
		{
		...
		{
			if (Keyboard::isKeyPressed(Keyboard::X) and canSpecial and !isAttack and !animation.playingAnimation and !Dead)
			{
				canSpecial = false;
				castStop = true;
	...
	// Player status -------------------
		...
		if (!cutScene)
		{
			if (stage == 0)
			{
				if (!Invincible)                      sprite.setFillColor(Color(235, 200, 130, 255));
				else if (Invincible and  bodyBlink) { sprite.setFillColor(Color(235, 200, 130,   0)); bodyBlink = false; }
				else if (Invincible and !bodyBlink) { sprite.setFillColor(Color(235, 200, 130, 255)); bodyBlink = true; }
			}
			else
			{
	...
	// Set Animation ---------------------
		...
		if (Dead)
		{
			if (charactor == 0) animation.OneTimeFrame.x = 9;
			else if (charactor == 1) animation.OneTimeFrame.x = 10;
			else if (charactor == 2) animation.OneTimeFrame.x = 12;
			else if (charactor == 3) animation.OneTimeFrame.x = 13;
			row = 5;
		...
		else if (getHit)
		{
			...
			else if (charactor == 1 or charactor == 3) animation.MaxFrame.x = 4;
		...
		else if (velocity.x == 0 and !jumping and !falling and !Dash and !castStop)
		{
			...
			else if (charactor == 1) animation.MaxFrame.x = 8;
		...
		{
			row = 6;
			if (charactor == 0 or charactor == 1 or charactor == 2) animation.MaxFrame.x = 6;
		...
		{
			row = 3;
			if (charactor == 0 or charactor == 1 or charactor == 2) animation.MaxFrame.x = 3;
		...
		{
			if (isAttack and combo == 1)      { animation.switchTime = 0.08; animation.OneTimeFrame.x = 9; }
			else if (isAttack and combo == 2) { animation.switchTime = 0.08; animation.OneTimeFrame.x = 9; }
			else if (isAttack and combo == 3) { animation.switchTime = 0.08; animation.OneTimeFrame.x = 6; }
			else if (isAttack and combo == 4) { animation.switchTime = 0.08; animation.OneTimeFrame.x = 6; }
		}
		else if (charactor == 2)
		{
			if (isAttack and combo == 1)      { animation.switchTime = 0.06; animation.OneTimeFrame.x = 9; }
			else if (isAttack and combo == 2) { animation.switchTime = 0.06; animation.OneTimeFrame.x = 9; }
			else if (isAttack and combo == 3) { animation.switchTime = 0.06; animation.OneTimeFrame.x = 12; }
			else if (isAttack and combo == 4) { animation.switchTime = 0.04; animation.OneTimeFrame.x = 12; }
		}
		else if (charactor == 3)
		{
			if (isAttack and combo == 1)	  { animation.switchTime = 0.08; animation.OneTimeFrame.x = 5; }
			else if (isAttack and combo == 2) { animation.switchTime = 0.08; animation.OneTimeFrame.x = 5; }
	...
	else if (charactor == 1)
	{
		...
		{
			sprite.setTexture(texture2);
			animation.imageCount = Vector2u(9, 4);
			sprite.setSize(Vector2f(140.0f, 130.0f));
			sprite.setOrigin(sprite.getSize() / 2.0f);
		...
		else
		{
			sprite.setTexture(texture);
			animation.imageCount = Vector2u(10, 7);
			sprite.setSize(Vector2f(160.0f, 130.0f));
			sprite.setOrigin(sprite.getSize() / 2.0f);
	...
	{
		if (faceRight) sprite.setPosition(position.x, position.y);
		else           sprite.setPosition(position.x, position.y);
	}
	else if (charactor == 2)
	{
		if (faceRight) sprite.setPosition(position.x, position.y-10);
		else           sprite.setPosition(position.x, position.y-10);
	}
	else if (charactor == 3)
	{
		if (faceRight) sprite.setPosition(position.x - 10.f, position.y - 10.f);
	...
	if (!attackBoxON) { attackBox.setSize(Vector2f(0,0));  attackBox.setPosition(0.0f, 9000.0f); }
	else if (attackBoxON and combo == 1)
	{
		...
		{
			attackBox.setSize(Vector2f(180.0f, 90.0f));
			attackBox.setOrigin(attackBox.getSize() / 2.f);
			if (animation.currentImage.x >= 6)
			{
				if (faceRight) attackBox.setPosition(position.x + 40.f, position.y);
				else		   attackBox.setPosition(position.x - 40.f, position.y);
			}
		}
		else if (charactor == 2)
		{
			attackBox.setSize(Vector2f(140.0f, 50.0f));
			attackBox.setOrigin(attackBox.getSize() / 2.f);
	...
	else if (attackBoxON and combo == 3)
	{
		...
		else if (charactor == 1)
		{
			...
			if (animation.currentImage.x >= 3)
			{
				if (faceRight) attackBox.setPosition(position.x+40, position.y);
				else		   attackBox.setPosition(position.x-40, position.y);
			}
		}
		else if (charactor == 2)
		{
	...
	else if (attackBoxON and combo == 4)
	{
		...
		{
			attackBox.setSize(Vector2f(120.0f, 120.0f));
			attackBox.setOrigin(attackBox.getSize() / 2.f);
			if (animation.currentImage.x >= 3)
			{
				if (faceRight) attackBox.setPosition(position.x, position.y-40);
				else		   attackBox.setPosition(position.x, position.y-40);
			}
		}
		else if (charactor == 2)
		{
