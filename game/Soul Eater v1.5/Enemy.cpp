...
Enemy::Enemy(Texture* texture, Texture* texture2, Vector2u imageCount, float x, float y, float switchTime, int type):
...
// Bullet --------------------
	else if (type == 7)
	{
		...
		hitBox.setSize(Vector2f(40.f, 15.f));
	}
// Rocket --------------------
	else if (type == 8)
	{
		sprite.setSize(Vector2f(60.f, 35.f));
		hitBox.setSize(Vector2f(60.f, 35.f));
	}
...
void Enemy::Update(float time, Vector2f position, Player& player, int type)
{
	...
	if (state != 0)
	{
		...
		if (type != 2 and type != 4 and type != 55 and type != 7 and type != 8 and state != 0 and state != 101)
		{
