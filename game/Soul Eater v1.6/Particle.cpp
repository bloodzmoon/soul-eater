...
void Particle::Update(Texture* texture, Player& player, float time, int type)
{
	...
	else if (type == 1)
	{
		...
		{
			if (player.faceRight)
				effect.setPosition(player.GetPosition().x + 50.0f, player.GetPosition().y);
			else
				effect.setPosition(player.GetPosition().x - 50.0f, player.GetPosition().y);
			setPosition = true;
		}
	...
	else if (type == 3)
	{
		if (isEffect and !setPosition)
		{
			if (player.charactor == 0 or player.charactor == 1)
			{
				if (player.faceRight)
				{
					faceRight = true;
	...
	else if (type == 4)
	{
		...
		if (isEffect)
		{
			...
			else if (player.charactor == 2)
			{
				...
				{
					faceRight = true;
					effect.setPosition(player.GetPosition().x - 60.f, player.GetPosition().y - 20.f);
				}
				else
				{
					faceRight = false;
