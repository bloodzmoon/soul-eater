...
void Interface::Update(Texture* texture, Vector2f position, float HP, float Energy, float time, bool stopMoving)
{
	...
	if (stopMoving)
	{
		soul.setPosition(savedCamera.x, position.y - 310.0f);
		healthBar.setPosition(savedCamera.x + 105.0f, position.y - 293.0f);
		energyBar.setPosition(savedCamera.x + 105.0f, position.y - 280.0f);
		EnemyCount.setPosition(savedCamera.x + 740.0f, position.y - 300.0f);
		
	}
	else 
	{
		soul.setPosition(position.x - 380.0f, position.y - 310.0f);
		healthBar.setPosition(position.x - 275.0f, position.y - 293.0f);
		energyBar.setPosition(position.x - 275.0f, position.y - 280.0f);
		EnemyCount.setPosition(position.x + 360.0f, position.y - 300.0f);

		savedCamera.x = position.x - 380.0f;
	}
