...
Interface::Interface(Texture* soulTex, Texture* enemyCounter, Font font, Vector2u imageCount, float switchTime):
	animation(soulTex, imageCount, switchTime)
{
	...
	EnemyIMG.setTexture(enemyCounter);
...
void Interface::Update(Texture* texture, Player player, View camera, float HP, float Energy, float time, bool cutScene)
{
	...
	else
	{
		...
		counter.setPosition(camera.getCenter().x + 355.f, camera.getCenter().y - 230.f);
	}
